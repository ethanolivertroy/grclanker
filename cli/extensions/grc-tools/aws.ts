...
function describeValueShape(value: unknown): string {
  ...
  switch (typeof value) {
    ...
    case "bigint":
      return "a number";
    case "object":
      return Object.keys(value).length === 0 ? "an empty structure" : "a structure";
    default:
      return typeof value;
  }
}
...
function observedXmlText(observed: ObservedResponse): string | undefined {
  ...
  while (offset < body.length && XML_WHITESPACE.has(body[offset])) offset += 1;
 ...
 */
function guardSdkClient<T extends object>(client: T): T {
  const prototype = Object.getPrototypeOf(client) as SdkClientPrototype;
  const streamCollector = asObject(asObject(client)?.config)?.streamCollector as StreamCollector | undefined;
...
export async function assessAwsDataProtection(
  ...
  // and an instance that did not report StorageEncrypted is never counted among those that reported it true.
