...
function scrubCarriers(text: string, pemScope: PemScope): string {
  ...
  const scrubbed = scrubHeaderLines(scrubPem(text, pemScope))
    ...
    .replace(QUOTED_ATTRIBUTE_PATTERN, (match, key: string, quote: string, value: string, offset: number, whole: string) => quotedValue(keyAfterEscape(whole, offset, key), value, () => `${key}=${quote}${REDACTION_MARKER}${quote}`, (url) => `${key}=${quote}${url}${quote}`, match));
