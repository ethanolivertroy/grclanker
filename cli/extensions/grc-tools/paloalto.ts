...
// "//" is a URL scheme, not a header. The token is looked for within
// FOLLOWING_HEADER_LOOKAHEAD characters of the separator.
...
function scrubCarriers(text: string, pemScope: PemScope): string {
  const scrubbed = scrubHeaderLines(scrubPem(text, pemScope))
    ...
    .replace(JSON_ESCAPED_PAIR_PATTERN, (match, run: string, key: string, separator: string, value: string) => (isCredentialKey(key) && !isCountValue(key, value) ? `${run}"${key}${run}"${separator}${run}"${REDACTION_MARKER}${run}"` : match))
    .replace(QUOTED_ATTRIBUTE_PATTERN, (match, key: string, quote: string, _value: string, offset: number, whole: string) => (isCredentialKey(keyAfterEscape(whole, offset, key)) ? `${key}=${quote}${REDACTION_MARKER}${quote}` : match));
