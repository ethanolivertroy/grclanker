...
test("CodeRabbit #81 quoted auth-params, end to end: a 401 body that echoes the request's Authorization header reaches the caller with the quoted token removed and no window of it", async () => {
  const client = new VeracodeApiClient(sampleConfig({ retries: 0 }), { fetchImpl: async () => jsonResponse({ http_code: 401, http_status: "Unauthorized", message: `Invalid API credentials. ${ECHOED_HEADER}` }, { status: 401 }), sleep: async () => {} });
  await assert.rejects(() => client.listPolicies(), (error) => {
    assert.equal(error.statusCode, 401);
    assert.match(error.message, /^Veracode request failed \(401[^)]*\) for \/appsec\/v1\/policies: Invalid API credentials\. /);
    assert.ok(error.message.endsWith(ECHOED_HEADER_REDACTED), `the echoed header reaches the caller redacted: ${error.message}`);
    assertNoWindowOf(error.message, QUOTED_TOKEN, "Veracode 401 body");
    return true;
  });
});
...
test("CodeRabbit #81 proof parameters, end to end: a 401 body that echoes a Digest header with a bare proof and a challenge-shaped list with a quoted one reaches the caller with both proofs removed and no window of them", async () => {
  const client = new VeracodeApiClient(sampleConfig({ retries: 0 }), { fetchImpl: async () => jsonResponse({ http_code: 401, http_status: "Unauthorized", message: `Invalid API credentials. ${ECHOED_PROOF}` }, { status: 401 }), sleep: async () => {} });
  await assert.rejects(() => client.listPolicies(), (error) => {
    assert.equal(error.statusCode, 401);
    assert.match(error.message, /^Veracode request failed \(401[^)]*\) for \/appsec\/v1\/policies: Invalid API credentials\. /);
    assert.ok(error.message.endsWith(ECHOED_PROOF_REDACTED), `the echoed proofs reach the caller redacted: ${error.message}`);
    assertNoWindowOf(error.message, PROOF_VALUE, "Veracode 401 body");
    return true;
  });
});
