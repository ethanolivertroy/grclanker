 ...
 * scheme word is read, so "sslPassword=splunk rejected" is a password whose value starts with a
 * product name, not a Splunk header. Scheme words act under Authorization-style keys and bare in
...
const SCHEME_ORDER_ROWS = [
  ...
  ["pass4SymmKey=splunk rejected", "pass4SymmKey=[REDACTED] rejected"],
  ["pass4SymmKey: splunk rejected", "pass4SymmKey: [REDACTED] rejected"],
  ['pass4SymmKey="splunk rejected"', 'pass4SymmKey="[REDACTED]"'],
  ['"pass4SymmKey": "splunk"', '"pass4SymmKey": "[REDACTED]"'],
  [`Authorization: splunk ${PLANTED_TOKEN}`, "Authorization: splunk [REDACTED]"],
...
test("CodeRabbit #81 quoted auth-params, end to end: a 401 body that echoes the request's Authorization header reaches the caller with the quoted token removed and no window of it", async () => {
  const client = new SplunkApiClient(sampleConfig(), { fetchImpl: async () => jsonResponse({ messages: [{ type: "ERROR", text: `call not properly authenticated. ${ECHOED_HEADER}` }] }, 401), retryDelayMs: 0 });
  await assert.rejects(() => client.listUsers(), (error) => {
    assert.equal(error.status, 401);
    assert.match(error.message, /^Splunk request to \/services\/authentication\/users failed \(401[^)]*\): call not properly authenticated\. /);
    assert.ok(error.message.endsWith(ECHOED_HEADER_REDACTED), `the echoed header reaches the caller redacted: ${error.message}`);
    assertNoWindowOf(error.message, QUOTED_TOKEN, "Splunk 401 body");
    return true;
  });
});
...
test("CodeRabbit #81 proof parameters, end to end: a 401 body that echoes a Digest header with a bare proof and a challenge-shaped list with a quoted one reaches the caller with both proofs removed and no window of them", async () => {
  const client = new SplunkApiClient(sampleConfig(), { fetchImpl: async () => jsonResponse({ messages: [{ type: "ERROR", text: `call not properly authenticated. ${ECHOED_PROOF}` }] }, 401), retryDelayMs: 0 });
  await assert.rejects(() => client.listUsers(), (error) => {
    assert.equal(error.status, 401);
    assert.match(error.message, /^Splunk request to \/services\/authentication\/users failed \(401[^)]*\): call not properly authenticated\. /);
    assert.ok(error.message.endsWith(ECHOED_PROOF_REDACTED), `the echoed proofs reach the caller redacted: ${error.message}`);
    assertNoWindowOf(error.message, PROOF_VALUE, "Splunk 401 body");
    return true;
  });
});
...
test("data-side ruling: vendor-prefixed tokens, JWTs, PEM blocks, and credential carriers are removed on the data side while identifiers and bare runs survive, in scrubDataText, in redactSnapshot, and in the exported Splunk bundle", async () => {
  ...
  assert.deepEqual(walked, {
    ...
    enabled: true,
    empty: null,
  }, "the walker reaches every string leaf; Splunk's deny list names webhook keys, so the URL goes whole");
  assert.deepEqual(redactSnapshot(walked), walked, "second pass over the walked record");

  const fixture = {
    ...HARDENED,
    "/services/apps/local": [
      entry("search", { author: "Splunk", label: "Search", version: "9.2.1", description: PLANTED_NOTE }),
      entry("splunk_monitoring_console", { author: "Splunk", label: "MC", version: "9.2.1" }),
    ],
  };
  assertFixtureFreeOfCanaryWindows(JSON.stringify({ fixture, config: sampleConfig() }), PLANTED_CANARIES, "data-side probe fixture");
  const secrets = leakWindows(PLANTED_CANARIES);
  const { fetchImpl } = createFetch(fixture);
  const api = new SplunkApiClient(sampleConfig(), { fetchImpl, retryDelayMs: 0 });
  const base = createTempBase("grclanker-splunk-data-side-");
  const result = await exportSplunkAuditBundle(api, sampleConfig(), base);
  const files = readBundleFiles(result.outputDir);
  assertSecretsAbsent(assert, files, secrets, "bundle directory");
  assertSecretsAbsent(assert, readZipEntries(result.zipPath), secrets, "zip archive");
  assertSecretsAbsent(assert, new Map([["assess results and access check", JSON.stringify([...(await runAllAssessments(api)), await checkSplunkAccess(api)])]]), secrets, "tool payloads");
