...
test("CodeRabbit #81 quoted auth-params, end to end: a 401 body that echoes the request's Authorization header reaches the caller with the quoted token removed and no window of it", async () => {
  const client = createRealClient(async () => new Response(JSON.stringify({ errorCode: "E0000011", errorSummary: `Invalid token provided. ${ECHOED_HEADER}`, errorCauses: [] }), { status: 401, statusText: "Unauthorized", headers: { "content-type": "application/json" } }));
