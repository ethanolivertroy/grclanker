...
/**
 * Round 4 item 2 (data-side carrier class): the healthy ServiceNow fixture with reviewer B's fifteen carrier
 ...
 */
test("rule 9 data side: a credential carried in any free-text field of any ServiceNow row never reaches a snapshot, evidence, summary, bundle file, or zip entry, while identifiers in the same fields stay", async () => {
  const configuredSecrets = [CONFIGURED_SECRET_CANARIES.servicenowPassword, CONFIGURED_SECRET_CANARIES.servicenowRefreshToken];
  const suffix = carrierSuffix(configuredSecrets);
  const harvest = async (injected) => {
    const config = sampleConfig({ password: configuredSecrets[0], refreshToken: configuredSecrets[1] });
    const { fetchImpl } = fixtureFetch(healthyFixture());
    const client = new ServicenowApiClient(config, { fetchImpl: injected ? injectingFetch(fetchImpl, suffix) : fetchImpl, sleep: async () => {}, now: () => FIXED_NOW });
    const access = await checkServicenowAccess(client);
    const assessments = await Promise.all([
      assessServicenowIdentityAccess(client),
      assessServicenowPlatformHardening(client),
    ...
    const base = createTempBase("grclanker-snow-text-fields-");
    try {
      const result = await exportServicenowAuditBundle(client, config, base);
      return [
