...
| 23 | License level verification | `elastic_assess_cluster_hardening` | `ELASTIC-23` | fail when the license tier does not cover configured features (SAML, OIDC, Kerberos, JWT, FLS, DLS need Platinum; LDAP, AD, PKI, audit logging, Watcher need Gold), the license status is `invalid` or `expired` (`active` and `valid` both count as active per the documented `LicenseStatus` values), or no license type is returned; warn for trial, expiry within 30 days, a missing expiry date, a missing status field, or when only some nodes responded |
