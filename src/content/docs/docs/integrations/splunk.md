...
| Data protection | `server.conf [sslConfig]` (`enableSplunkdSSL`, `sslVersions`, `cipherSuite`, `requireClientCert`), `web.conf [settings] enableSplunkWebSSL`, `outputs.conf [tcpout:*]`, `/services/data/inputs/tcp/ssl`, `/services/data/inputs/http` (global `[http]` `disabled` and `enableSSL`, per-token `indexes`, `sourcetype`, `useACK`, `disabled`), ACS `/inputs/http-event-collectors` on Splunk Cloud |
...
All tools accept the shared connection arguments (`url`, `token`, `username`, `password`, `stack`, `acs_token`, `acs_base_url`, `verify_ssl`, `config_file`, `timeout_seconds`). Every list endpoint is paged with `output_mode=json`, `count`, and `offset` until `paging.total` is reached; if the total is larger than the entries returned, the verdict is downgraded and the seen/total counts are reported.
...
| 6 | Authentication token hygiene | `splunk_assess_authentication` | `SPLUNK-AUTH-06` | `pass` when every token expires, is newer than `max_token_age_days`, and maps to a known user; `warn` for stale tokens, tokens missing `iat`/`exp` claims, or a partial list; `fail` for non-expiring or orphaned tokens; `manual` when the token list is unreadable or empty |
...
| 9 | Search head access controls | `splunk_assess_access_control` | `SPLUNK-AC-09` | `pass` when no non-admin role has `srchIndexesAllowed` containing `*` without an `srchFilter`; `fail` otherwise; `manual` when no role exposes the field |
| 10 | Index access control | `splunk_assess_access_control` | `SPLUNK-AC-10` | `pass` when only admin roles can search `_audit` and `_internal` (imported roles included); `fail` otherwise |
...
| 13 | TLS/SSL configuration | `splunk_assess_data_protection` | `SPLUNK-DP-13` | `pass` when `enableSplunkdSSL` and `enableSplunkWebSSL` are on, `sslVersions` excludes TLS below 1.2, and `requireClientCert=true`; `warn` when `requireClientCert` is not enabled; `fail` when SSL is off, `enableSplunkWebSSL` is absent (documented default false), or legacy TLS is allowed; `manual` when `[sslConfig]` or `web.conf` is unreadable |
| 14 | Data encryption at rest | `splunk_assess_data_protection` | `SPLUNK-DP-14` | always `manual`: Splunk Enterprise has no index-level encryption setting (collect volume encryption evidence for `homePath`/`coldPath`), and the ACS EMEK endpoints (`/emek/key-policy`, `/emek/waiver`, `/emek/key`) generate onboarding artifacts rather than reporting whether EMEK is active (collect the EMEK provisioning record or encryption attestation) |
| 15 | Forwarding encryption | `splunk_assess_data_protection` | `SPLUNK-DP-15` | `pass` when every `outputs.conf [tcpout:*]` group declares `useSSL`, `clientCert`, or `sslPassword`; `fail` when any group has none; `manual` on Splunk Cloud, when `outputs.conf` is unreadable, or when the node has no tcpout groups |
| 16 | HEC token security | `splunk_assess_data_protection` | `SPLUNK-DP-16` | `pass` when `[http] enableSSL=true` is read explicitly and every enabled token restricts indexes, enables `useACK`, and sets a sourcetype (or when HEC is globally disabled with no tokens); `warn` for missing `useACK`/sourcetype, absent `enableSSL`, or truncation; `fail` for `enableSSL=false` or any-index tokens; `manual` when inputs are unreadable, the global `[http]` entry is missing, or no tokens are visible |
...
| 22 | Saved search permissions | `splunk_assess_platform_hardening` | `SPLUNK-PLAT-22` | `pass` when no scheduled search dispatches as an admin owner over all indexes with an unbounded time range (or none are scheduled); `warn` when admin-owned searches dispatch as owner or owner roles could not be verified; `fail` otherwise; `manual` when saved searches are unreadable or empty |
| 23 | Splunk-to-Splunk port security | `splunk_assess_platform_hardening` | `SPLUNK-PLAT-23` | `pass` when every enabled `splunktcp` listener uses TLS and `[SSL] requireClientCert=true`; `warn` when listeners use TLS without client certificates; `fail` for plaintext listeners; `manual` on Splunk Cloud or when this node has no receiving ports |

Common verdict rules apply to every control: an unreadable or forbidden endpoint is `manual`, an empty inventory is `manual` (or `fail` where the control's intent requires the inventory), a partial inventory (`paging.total` greater than the entries returned) never yields `pass`, and items without dates are reported separately and cap the verdict at `warn`.
...
- Controls 15 and 23 (forwarder and S2S TLS) read `outputs.conf` and `splunktcp` inputs on the node you connect to. On Splunk Cloud, or on a search head that neither forwards nor receives, they are `manual` and name the forwarder or indexer files to collect.
...
- The spec lists `sslCertPath` for forwarder TLS; current `outputs.conf` documents `clientCert`, so both names are accepted.
...
- [REST API Reference: input endpoints (`data/inputs/http`, `data/inputs/tcp/cooked`, `data/inputs/tcp/ssl`)](https://docs.splunk.com/Documentation/Splunk/latest/RESTREF/RESTinput)
