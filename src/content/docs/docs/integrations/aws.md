...
| 4 | Root Account Usage | `aws_assess_identity` | `AWS-IAM-01`, `AWS-IAM-07` | `AWS-IAM-07` reads CloudTrail `LookupEvents` with `Username=root` over `lookback_days` in the configured region: `manual` when the lookup is denied; `fail` on any `ConsoleLogin` event; `warn` on other root API events; `pass` when none exist. Truncated lookups and events without `EventTime` cap at `warn`. Console sign-ins are global events recorded in `us-east-1`. |
...
- Root sign-in events are global CloudTrail events recorded in `us-east-1`; run `aws_assess_identity` with `region=us-east-1` for console sign-in coverage. `LookupEvents` keeps 90 days of history.
