...
| 9 | GuardDuty Enabled | `aws_assess_logging_detection` | `AWS-LOG-04` | `manual` when `ListDetectors` is denied, or when detectors exist but `GetDetector` could not be read for any of them; `fail` unless at least one detector reports `Status=ENABLED` in the configured region; a `GetDetector` denial on any other detector or a truncated detector list caps `pass` at `warn`. `guardduty_detectors` and `enabled_guardduty_detectors` are `null` when the list, or every detector's detail, could not be read. Protection-plan features are not yet read (see Limitations). |
