...
9. The bundle never carries credential-bearing values: access key ids are masked, the security contact's name, title, email, and phone are reduced to presence flags and the email domain, and policy documents are summarized as flags. Error lines quote the AWS error code and message only.
...
| 3 | Access Key Rotation | `aws_assess_identity` | `AWS-IAM-04` | `manual` when `ListUsers` is denied or `ListAccessKeys` failed for every sampled user; `fail` when any key's `GetAccessKeyLastUsed.LastUsedDate` (or `CreateDate` when never used or when the last-used read failed) is older than `stale_days`; evidence carries masked key ids only. A `ListAccessKeys` or `GetAccessKeyLastUsed` denial on any other user or key, or a truncated user inventory, caps `pass` at `warn` naming it. |
