...
Each finding has the shape `{ id, control, title, severity, status, summary, evidence, mappings }` where `severity` is `critical`, `high`, `medium`, `low`, or `info` and `status` is `pass`, `warn`, `fail`, or `manual`. A `manual` finding states exactly what evidence a human must collect from Anypoint Platform.
...
| --- | --- | --- | --- | --- |
| 1 | SSO/SAML or OIDC external identity provider configured | identity_access | `MULESOFT-IAM-01` | fail: no identity provider; warn: provider present but new non-SSO users allowed; pass otherwise |
| 2 | MFA enforced for all organization members | identity_access | `MULESOFT-IAM-02` | fail: users listed with `mfaVerificationExcluded=true`; manual otherwise (capture the organization MFA setting or IdP MFA policy) |
| 3 | Organization Administrator membership minimized | identity_access | `MULESOFT-IAM-03` | pass: distinct members of admin role groups within `max_admins` (default 5); fail above; warn when no admin role group is visible |
| 4 | Role groups follow least privilege | identity_access | `MULESOFT-IAM-04` | fail: custom role group grants Organization Administrator or Owner; warn: more than `max_roles_per_group` roles or no role groups visible; pass otherwise |
| 5 | Role groups scoped to specific environments | identity_access | `MULESOFT-IAM-05` | pass: every environment-level role carries `context_params.envId`; warn: 1-5 org-wide environment roles; fail: more than 5 |
| 6 | Production and sandbox environments isolated | identity_access | `MULESOFT-IAM-06` | fail: environment names contradict their type; warn: no production or no sandbox environment; pass otherwise |
| 7 | Authentication policies on production APIs | api_gateway | `MULESOFT-API-07` | fail: a production API instance has no active authentication policy; warn: only non-production instances lack one, or no APIs visible; pass otherwise |
| 8 | Rate limiting policies applied | api_gateway | `MULESOFT-API-08` | fail: production instance without rate limiting or spike control; warn: non-production only or no APIs visible; pass otherwise |
| 9 | Client credentials rotated within policy period | api_gateway | `MULESOFT-API-09` | manual: the API does not expose secret rotation timestamps; export contracts and client applications and confirm reset dates |
| 10 | Supported Mule runtime versions | runtime_infrastructure | `MULESOFT-RT-10` | fail: Mule 3 or `endOfSupportDate` in the past; warn: end of support within `runtime_support_warning_days` or unknown; pass otherwise |
| 11 | Worker sizing reviewed | runtime_infrastructure | `MULESOFT-RT-11` | warn: multiple or large workers in non-production, or four or more workers with CPU under 10 percent; pass otherwise |
| 12 | Persistent queues encrypted | runtime_infrastructure | `MULESOFT-RT-12` | fail: `persistentQueues` enabled without `persistentQueuesEncrypted`; pass otherwise |
| 13 | VPC firewall rules restrictive | runtime_infrastructure | `MULESOFT-RT-13` | fail: rule allows all protocols; warn: wide port ranges or CIDR broader than /16; manual: no VPC visible; pass otherwise |
| 14 | No 0.0.0.0/0 VPC ingress | runtime_infrastructure | `MULESOFT-RT-14` | fail: open ingress on ports other than 8081, 8082, 8091, 8092; warn: open ingress only on those CloudHub listener ports; manual: no VPC visible; pass otherwise |
| 15 | DLB enforces TLS 1.2+ | runtime_infrastructure | `MULESOFT-RT-15` | fail: `tlsv1` enabled; warn: `httpMode` is `on` (plain HTTP served); pass otherwise or when no DLB exists |
| 16 | DLB certificates valid beyond 30 days | runtime_infrastructure | `MULESOFT-RT-16` | fail: live TLS probe shows expiry within 30 days; warn: within `certificate_warning_days` (default 60); manual: probe failed; pass otherwise |
| 17 | Audit logging active and queryable | audit_monitoring | `MULESOFT-AUD-17` | pass: entries within `audit_lookback_hours` (default 24); warn: entries only within 7 days; fail: query failed or no entries |
| 18 | Connected apps use minimum scopes | identity_access | `MULESOFT-IAM-18` | fail: client credentials app holds `full`, admin, owner, or manage scopes; warn: user-delegated app requests `full` or more than `max_connected_app_scopes`; pass otherwise |
| 19 | Stale connected apps reviewed | identity_access | `MULESOFT-IAM-19` | warn: unused for `stale_connected_app_days` (default 90) or disabled; manual: no usage data returned; pass otherwise |
| 20 | Exchange assets follow governance review | api_gateway | `MULESOFT-API-20` | warn: assets published to the public portal; manual otherwise (export the API Governance conformance report and publishing settings) |
| 21 | Anypoint MQ access restricted by environment | runtime_infrastructure | `MULESOFT-RT-21` | warn: unencrypted queues; manual: inventory returned, confirm client apps are not shared across environments; pass when MQ is unused |
| 22 | Secrets Manager used for sensitive configuration | runtime_infrastructure | `MULESOFT-RT-22` | fail: sensitive-looking CloudHub properties not marked secure; warn: production environment without secret groups; pass otherwise |
| 23 | Hybrid runtime servers registered and reporting | runtime_infrastructure | `MULESOFT-RT-23` | fail: all servers disconnected; warn: some not `RUNNING`; pass otherwise or when none registered |
| 24 | Alerts configured for production applications | audit_monitoring | `MULESOFT-AUD-24` | fail: production environment with no enabled CloudHub or Runtime Manager alert; warn: applications not covered or no production environment sampled; pass otherwise |
| 25 | Business groups separate tenants | identity_access | `MULESOFT-IAM-25` | pass: business groups exist under the organization; manual otherwise (confirm whether tenants share the root organization) |
...
- The dedicated load balancer API does not return certificate validity, so control 16 performs a live TLS handshake against each DLB domain with `node:tls`; hosts that are not reachable from the auditor produce a manual finding.
...
- [VPC Firewall Rules](https://docs.mulesoft.com/cloudhub/vpc-firewall-rules-concept), [Dedicated Load Balancers](https://docs.mulesoft.com/cloudhub/cloudhub-dedicated-load-balancer), and [SSL Endpoints and Certificates](https://docs.mulesoft.com/cloudhub/lb-ssl-endpoints)
