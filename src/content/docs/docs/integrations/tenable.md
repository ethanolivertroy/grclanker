...
|---|---------|------|---------|-----------|
| 1 | Scan policy configuration | scan_program | TENABLE-01 | `manual` when scans exist (port range, plugin family, and safe-check settings are not in the published policy schema, so template usage is listed for UI review); `fail` when zero scans are visible or every scan uses a discovery template. |
...
| 8 | Plugin update currency | sensor_coverage | TENABLE-08 | Container `plugin_set` and each linked scanner `loaded_plugin_set` newer than `plugin_stale_hours`; unreadable or empty scanner lists cap at `warn`. |
...
| 10 | User role and permission audit | access_control | TENABLE-10, TENABLE-10-SC | Reads `enabled`, `permissions`, `lastlogin`, `two_factor`, `ui_saml_only`, `login_fail_count`, `locked`, and `last_apikey_access`; excessive admins, admins without MFA or SAML, or inactive enabled users `fail`; zero users `manual`. |
| 11 | Access group review | access_control | TENABLE-11 | Permissions granting AllUsers write-style actions on all assets `fail`; remaining legacy access groups `warn`; zero permissions `manual`. |
...
| 19 | Export and reporting automation | vulnerability_management | TENABLE-19 | Export jobs other than this assessment in the last 30 days; report schedules are not exposed by the API and remain a manual check. |
...
- Scan policy settings (port range, plugin families, safe checks, performance) are not part of the published `GET /policies/{policy_id}` schema, so control 1 is always `manual` when scans exist.
- Report schedules and Lumin exposure metrics have no documented read endpoint; control 19 relies on export job history only.
...
- [Retrieve vulnerability data (export workflow)](https://developer.tenable.com/docs/retrieve-vulnerability-data-from-tenableio), [Export vulnerabilities](https://developer.tenable.com/reference/exports-vulns-request-export), and [Export assets](https://developer.tenable.com/reference/export-assets-v2)
- [List scans](https://developer.tenable.com/reference/scans-list) and [List users](https://developer.tenable.com/reference/users-list)
