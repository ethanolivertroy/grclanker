...
Requests carry the documented header `X-ApiKeys: accessKey={access_key};secretKey={secret_key}`. Keys are never written to disk. Every error string passes through one redacting sink before it reaches a finding, a tool result, or a bundle file: the configured keys are replaced, credential-shaped text an upstream body may echo (bearer or basic tokens, `Authorization`, `Cookie`, and `X-ApiKeys` header lines, `token=`, `api_key=`, `session=`, and similar pairs, JWTs, and URL userinfo) is replaced with `[REDACTED]`, and a non-JSON error body (an HTML 502 page, a WAF block, an SSO interstitial) is never echoed; the message records only the status and the body length, for example `403; non-JSON text/html response body (1204 bytes, not echoed)`. JSON error bodies contribute their documented `error`, `error.message`, `message`, or `error_msg` fields only, after the same scrub.
...
`core_data/` is not a verbatim dump. `policy_details.json` keeps only `uuid`, `name`, `settings`, and `plugins` from `GET /policies/{policy_id}`; the `credentials`, `audits`, and `scap` blocks are dropped before storage. `scanners.json` replaces each scanner's `key` (linking key), `registration_code`, and `license` block with `[REDACTED]`. Every collected dataset then passes through a property-name redaction that replaces the value of any property whose name denotes a credential (`password`, `secret`, `token`, `api_key`, `access_key`, `secret_key`, `private_key`, `client_secret`, `registration_code`, `authorization`, `cookie`, and their camelCase forms), the `value` of `{name, value}` pairs whose `name` matches (activity log `fields`, for example), and credentials embedded in string values (`?token=`, `?api_key=`, `?secret=` query parameters and `https://user:pass@` userinfo). The same redacted objects feed the findings and tool results, so nothing the assessments return can carry a value the bundle would not.
