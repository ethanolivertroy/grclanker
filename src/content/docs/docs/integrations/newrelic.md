...
- NerdGraph queries are POSTed to `/graphql` and paginated with `nextCursor` where the schema exposes a cursor. Pagination runs to completion or the list is marked incomplete with the number of items seen and the reported total, and the affected findings record the truncation. `apiAccess.keySearch` is tried with a cursor first and falls back to a single page if the schema rejects the argument; the fallback is recorded as an incomplete listing.
...
- `analysis/findings.json` plus `analysis/identity.json`, `analysis/access_control.json`, `analysis/alerting.json`, and `analysis/data_governance.json`; each category file lists its `coverage` limitations (truncated lists, unreadable scopes, fallback pagination).
...
2. An empty inventory never yields `pass` by default. Each summary states whether emptiness is unknown (`manual`) or a violation (`fail`, for example zero destinations while alert policies exist). The two accepted empty passes state their conditions in the summary: control 6 passes on zero keys only when `keySearch` was readable and complete for every account in scope and every in-scope account is visible in `actor.accounts`; control 20 passes on zero custom roles only when the role listing was readable, complete, and returned standard roles.
...
5. A partial view (a key scoped to some accounts, the `keySearch` single-page fallback, sampled synthetic scripts at `script_sample_limit`, an `entitySearch` or user list that stopped before `nextCursor` was exhausted, or an unreadable account or authentication domain among several) limits an otherwise passing finding to `warn`. The summary appends a `Partial view:` clause with the seen and total counts, and the assessment result lists every limitation under `coverage`.
6. Every flag a verdict depends on is read explicitly: `authenticationType`, user `type`, group membership and role grants, condition `enabled`, `workflowEnabled`, destination `type` and `active`, obfuscation rule `enabled`, `retentionInDays` per namespace, dashboard `permissions`, and `monitorType`. A value whose enabling flag is `false` or absent never supports `pass`.
...
| 4 | API Key Inventory | `newrelic_assess_access_control` | `NR-04-API-KEY-INVENTORY` | `manual` when keys, users, or group grants are unreadable, or zero keys are returned (every account has its original license key). `warn` if keys are unnamed, expose no type, belong to admin group members, or admin detection is incomplete. `pass` otherwise. |
...
| 6 | Unused API Keys | `newrelic_assess_access_control` | `NR-06-UNUSED-API-KEYS` | `warn` for user keys owned by missing, inactive, or undated users. `manual` when keys exist without owner signals, because `NrAuditEvent` records configuration changes only, or when zero keys are returned but coverage is unconfirmed. `pass` only on zero keys when `keySearch` was complete for every account in scope and every in-scope account is visible; the summary states both conditions. |
...
| 10 | Alert Notification Channels | `newrelic_assess_alerting` | `NR-10-ALERT-NOTIFICATION-CHANNELS` | `manual` when destinations, channels, or workflows are unreadable, or when nothing exists and policies are absent or unreadable. `fail` for personal email destinations or zero destinations while policies exist. `warn` for unapproved domains, no workflow with `workflowEnabled = true`, inactive destinations, destinations missing `type` or `active`, or no approved domain list. `pass` otherwise. |
...
| 12 | Log Obfuscation Rules | `newrelic_assess_data_governance` | `NR-12-LOG-OBFUSCATION` | `manual` when rules are unreadable, or when no rules exist and no logs were ingested (not applicable). `fail` with no rule that has `enabled = true`. `warn` if expressions are unreadable, do not cover both credential and PII patterns, or rules lack the `enabled` flag. `pass` otherwise. Pipeline Control unavailability is reported in evidence, not counted. |
...
| 20 | Custom Role Permissions | `newrelic_assess_access_control` | `NR-20-CUSTOM-ROLE-PERMISSIONS` | `manual` when roles are unreadable, zero roles are returned, custom roles exist (NerdGraph does not expose role capabilities), or roles expose no type. `warn` if the role listing was incomplete. `pass` only when the listing was readable and complete and returned standard roles; the summary states both conditions. |

Every `pass` above additionally requires a complete inventory: when a list was truncated, a scope was unreadable, or `keySearch` fell back to a single page, the finding is limited to `warn` and the summary carries a `Partial view:` clause.

Coverage: 20 of 20 controls. Controls 16, 17, and 18 never pass by design, control 20 is manual whenever custom roles exist, and control 6 is manual whenever keys exist, because `NrAuditEvent` cannot prove read-only key usage. Each `manual` summary states the UI evidence to collect.
