...
| 14 | NSG rules | guardrails | OCI-GRD-02 | same test on INGRESS `SecurityRule`s with `isValid` recorded per rule; no NSGs passes only beside readable security lists |
