...
- Pagination follows the Table API's `Link rel=next` header on the instance's own origin only: a link whose origin differs from the configured instance URL is never requested (the Authorization header would otherwise travel to that host), and the read is reported `truncated` with `truncation_reason` "Link rel=next pointed at another origin; not followed", as a stuck or empty next page is with its own reason.
