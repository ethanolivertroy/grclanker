...
| 2 | ACL rule completeness | access_control | SNOW-02 | `sys_security_acl`, `sys_security_acl_role`, `sys_public` (plus the `sys_security_acl` Aggregate count) | manual unless both hold: the Aggregate ACL count was returned and is above zero, and the Table API read returned ACL rows (the count alone is never proof of visibility: a positive count with no visible rows is the ACL filtering described under Limitations and stays manual, "none of the sensitive-table or wildcard ACLs were visible"); fail on role-less, condition-less, script-less record ACLs; warn on wildcard ACLs or active public pages; pass only when the visible rows also match `X-Total-Count`. Manual when any input is unread; an ACL read that was truncated, lacks `X-Total-Count`, or returned fewer rows than the header reports turns pass into warn (a fail on a visible ACL stands) |
