...
| 10 | Incident response automation configured | `pagerduty_assess_incident_response` | `PD-10` | `pass` with at least one `is_enabled: true` workflow and one trigger with `is_disabled: false` and no trigger missing the flag, `warn` when workflows, triggers, or legacy response plays exist but none is verified enabled (including a trigger without the `is_disabled` flag), `fail` when the readable endpoints return none, `manual` on plan or read errors; the summary states enabled, disabled, and unflagged trigger counts |
