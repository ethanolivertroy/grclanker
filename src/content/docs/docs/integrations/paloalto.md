...
| `paloalto_export_audit_bundle` | Runs everything and writes `core_data/`, `analysis/`, `compliance/`, `QUICK_REFERENCE.md`, `_errors.log` (only on partial failure), and a zip archive under `./export/paloalto` by default. The PAN-OS configuration in `core_data/` is stored with credential-bearing nodes (`phash`, `password`, `secret`, `key` and `pre-shared-key`, `private-key`, `passphrase`, `token`, SNMPv3 `authpwd` and `privpwd`, `snmp-community-string`, and similar names) replaced by `[REDACTED]`; settings such as `password-complexity` and `public-key` are kept, and the assessments still evaluate the raw values in memory. |
