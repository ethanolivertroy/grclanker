...
Configured secrets (`PRISMA_SECRET_KEY`, `PANOS_API_KEY`, `PANOS_PASSWORD`, and the JWTs and Compute tokens the clients obtain) never appear in tool output, findings, `_errors.log`, or bundle files. Every error string is built once and scrubbed at the single sink every collector, access probe, and tool catch block reads through: the configured values are replaced, and an unanchored pass replaces credential-shaped text an upstream body may echo regardless of what this tool sent (`Bearer` and `Basic` values; `Authorization`, `Cookie`, `Set-Cookie`, `X-Api-Key`, `X-PAN-KEY`, and `x-redlock-auth` header lines; `key=value`, `key: value`, and `"key":"value"` pairs whose key contains `session`, `token`, `secret`, `password`, `api_key`, `signature`, or `authorization`; PAN-OS `LUFRPT...` keys; JWTs; and URL userinfo) with `[REDACTED]`. Redaction runs before any length cap, so a value cut by the cap can never survive as a prefix.
