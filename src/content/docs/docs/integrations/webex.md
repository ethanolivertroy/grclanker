...
- Identity: Compliance Officer assignment, administrator concentration, bot account inventory, plus manual evidence prompts for SSO, admin MFA, and bot approval.
...
- Meeting and hybrid security: hybrid connector health, plus manual prompts for E2EE and SRTP defaults, lobby and password defaults, guest and virtual background policy, and device firmware posture (with inventory evidence).
...
Scopes for full coverage: `spark-admin:people_read`, `spark-admin:organizations_read`, `spark-admin:roles_read`, `spark-admin:licenses_read`, `spark-admin:devices_read`, `spark-admin:hybrid_clusters_read`, `spark-admin:workspaces_read`, `spark-compliance:events_read`, `spark-compliance:recordings_read`, `audit:events_read`, `meeting:schedules_read`, `meeting:preferences_read`, `spark:rooms_read`, and `spark:webhooks_read`. Compliance scopes require the Compliance Officer role.
...
| `webex_assess_identity` | WEBEX-ID-01 to WEBEX-ID-06. |
| `webex_assess_collaboration_governance` | WEBEX-COLLAB-01 to WEBEX-COLLAB-08. |
| `webex_assess_meeting_hybrid_security` | WEBEX-MTG-01 to WEBEX-MTG-05. |
...
- `manual`: the endpoint was denied or errored, the population was empty where emptiness is inconclusive, or the setting is only visible in Control Hub. The summary names the cause and the evidence to collect.
...
| 2 Admin MFA | identity | WEBEX-ID-02 | manual: People API exposes no MFA field; admin list attached |
| 3 Compliance Officer role | identity | WEBEX-ID-03 | pass/fail/warn from `/people` roles and `/roles` names |
| 4 External communications | collaboration | WEBEX-COLLAB-01 | manual: Control Hub setting |
...
| 8 E2E meeting encryption | meeting-hybrid | WEBEX-MTG-01 | manual: no encryption field in meeting preferences |
| 9 Meeting lobby | meeting-hybrid | WEBEX-MTG-02 | manual: org default not exposed; sampled `unlockedMeetingJoinSecurity` |
| 10 Meeting password | meeting-hybrid | WEBEX-MTG-02 | manual: org default not exposed; sampled `password` |
...
| 13 Guest access | collaboration, meeting-hybrid | WEBEX-COLLAB-01, WEBEX-MTG-03 | manual: Control Hub setting |
...
| 17 Device firmware | meeting-hybrid | WEBEX-MTG-05 | manual: no end-of-life field; `software` versions inventoried |
...
| 23 Virtual background | meeting-hybrid | WEBEX-MTG-03 | manual: Control Hub setting |
...
Automatable controls: 3, 14, 15, 16, 19 (inventory), 20, 24, 25. All 25 controls have a finding.
...
- Reference pages on developer.webex.com are client-rendered; field names were taken from those reference pages and cross-checked against the rendered guides (basics, compliance, bots, integrations, Service Apps).
...
| `GET /people/me` | `id`, `displayName`, `emails`, `type` | [Get My Own Details](https://developer.webex.com/docs/api/v1/people/get-my-own-details) |
| `GET /people?orgId&max` | `id`, `displayName`, `emails`, `roles`, `type`, `created` | [List People](https://developer.webex.com/docs/api/v1/people/list-people) |
| `GET /organizations` | `id`, `displayName` | [List Organizations](https://developer.webex.com/docs/api/v1/organizations/list-organizations) |
| `GET /organizations/{orgId}` | `id`, `displayName`, `created` | [Get Organization Details](https://developer.webex.com/docs/api/v1/organizations/get-organization-details) |
| `GET /roles` | `id`, `name` | [List Roles](https://developer.webex.com/docs/api/v1/roles/list-roles) |
| `GET /licenses?orgId` | `totalUnits`, `consumedUnits` | [List Licenses](https://developer.webex.com/docs/api/v1/licenses/list-licenses) |
| `GET /events` | count, `created` | [List Events](https://developer.webex.com/docs/api/v1/events/list-events), [Compliance guide](https://developer.webex.com/docs/api/guides/compliance) |
| `GET /adminAudit/events?orgId&from&to&max` | `created` | [List Admin Audit Events](https://developer.webex.com/docs/api/v1/admin-audit-events/list-admin-audit-events) |
| `GET /admin/recordings` | `status` | [List Recordings For an Admin or Compliance Officer](https://developer.webex.com/docs/api/v1/recordings/list-recordings-for-an-admin-or-compliance-officer) |
| `GET /meetings` | `password`, `unlockedMeetingJoinSecurity` | [List Meetings](https://developer.webex.com/docs/api/v1/meetings/list-meetings) |
| `GET /meetingPreferences` | `personalMeetingRoom.enabledAutoLock` | [Get Meeting Preference Details](https://developer.webex.com/docs/api/v1/meeting-preferences/get-meeting-preference-details) |
| `GET /meetingPreferences/sites` | `siteUrl` | [Get Site List](https://developer.webex.com/docs/api/v1/meeting-preferences/get-site-list) |
| `GET /hybrid/clusters?orgId` | `id`, `name` | [List Hybrid Clusters](https://developer.webex.com/docs/api/v1/hybrid-clusters/list-hybrid-clusters) |
| `GET /hybrid/connectors?orgId` | `id`, `type`, `status`, `version`, `createdAt` | [List Hybrid Connectors](https://developer.webex.com/docs/api/v1/hybrid-connectors/list-hybrid-connectors) |
| `GET /devices?orgId` | `software`, `managedBy`, `personId`, `workspaceId` | [List Devices](https://developer.webex.com/docs/api/v1/devices/list-devices) |
| `GET /workspaces?orgId` | count | [List Workspaces](https://developer.webex.com/docs/api/v1/workspaces/list-workspaces) |
| `GET /rooms` | `id`, `title`, `classificationId` | [List Rooms](https://developer.webex.com/docs/api/v1/rooms/list-rooms) |
| `GET /webhooks` | `id`, `name`, `targetUrl`, `secret`, `status` | [List Webhooks](https://developer.webex.com/docs/api/v1/webhooks/list-webhooks) |

Pagination follows the RFC 5988 `Link` header with `rel="next"` until absent and reports truncation when an item limit stops early; 429 responses honor `Retry-After` (both on the [basics](https://developer.webex.com/docs/api/basics) page).
