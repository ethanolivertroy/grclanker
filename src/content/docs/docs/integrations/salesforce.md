...
| 6 | Login hour restrictions | identity_access | SF-06 | pass when every sensitive profile restricts `loginHours` (a 0 to 1440 window on every day is not a restriction); warn when some do or a profile could not be resolved; fail when none do; manual when `readMetadata(Profile)` is forbidden |
