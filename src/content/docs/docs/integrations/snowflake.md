...
Statements are submitted with `POST /api/v2/statements?async=true` and polled with `GET /api/v2/statements/{handle}` while the API returns `202`, honoring `Retry-After`. The poll follows the `statementStatusUrl` the API returned only when it resolves onto the configured account origin (scheme, host, and port): a value that would move the host is never requested, the statement fails with the fixed text `Snowflake SQL API returned a statement status URL that is not on the configured account origin; the statement was not polled and its result was not read.`, and the outcome is recorded as `error` with that reason, so the bearer token never leaves for another host and no part of the returned URL reaches an error string. The request layer refuses any URL off that origin the same way (`Snowflake SQL API request refused: the request URL is not on the configured account origin, so no request was sent.`). The response that completes the statement carries partition 0 in its `data`; when `resultSetMetaData.partitionInfo` lists more than one partition, each additional partition (1 through the last, within `max_partitions`) is fetched with `GET /api/v2/statements/{handle}?partition=N`. `429` and `5xx` responses are retried with exponential backoff, HTTP timeouts abort the request, and error messages are scrubbed as described under Export bundle.
