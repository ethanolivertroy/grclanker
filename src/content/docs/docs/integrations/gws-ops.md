...
Every workflow accepts `dry_run: true` to print the exact command without executing it. A dry run renders `null` for every field a real call would derive from records (`count`, `complete`, `next_page_token`, `records`, and the raw response) beside `status: not collected: dry run previewed the command and did not execute it, so no records were requested`; it never reports `count: 0` or `records: []` for a call that did not happen. An executed call carries the counts plus a `status` that reads `complete: the CLI response carried no nextPageToken, so the N record(s) are the whole population for this query` or `partial: at least N; the CLI response carried a nextPageToken after N record(s), so more records exist beyond this page`. `max_results` is clamped to 1-250 and `lookback_days` to 1-90; when a request exceeds 250 (for example 1000, the `activities.list` maximum), the notes state the requested value and the clamp rather than silently reporting 250.
...
- Error text hygiene: the CLI's stderr and every error string the bridge builds pass through one function, `scrubCliText` in `cli/extensions/grc-tools/gws-ops.ts`, which applies the inspector's text scrubber (bearer credentials, `ya29.` and `1//` tokens, `AIza` keys, `GOCSPX-` secrets, JWTs, PEM blocks, `key=value` and `key: value` credential pairs, URL query strings) and then replaces every `GOOGLE_WORKSPACE_CLI_*` token or secret value from the environment. `GwsCliCommandError` scrubs its own message in the constructor, the access probe scrubs the stderr it reports, a zero exit whose stdout is not JSON is described without repeating that stdout, and every tool error result is scrubbed once more where it is built, so no consumer of the bridge can receive an unscrubbed string. `cli/tests/gws-ops.test.mjs` plants an env token, a `Bearer ya29.` value, and an `access_token=` pair in stderr and asserts none reaches the thrown error, the tool result, or the probe. The `--version` output, the one CLI-produced string that reaches a tool result without a JSON projection, passes through `scrubCliText` and then only its first line is matched against the documented `gws <version>` line (the binary prints two lines, `gws <version>` and a fixed disclaimer): `gws_ops_check_cli` renders the numeric core alone (`gws 0.22.5`; a pre-release or build suffix is accepted but never rendered), the version number plus a note when the first line is not the documented one but a version number sits in the output, and a descriptor giving only the output length when no version number is present, so nothing the binary prints beyond `major.minor.patch` can reach the tool text or JSON.
...
| --- | --- | --- | --- |
| `gws --version` | [README, Quick Start](https://github.com/googleworkspace/cli#readme); `crates/google-workspace-cli/src/main.rs` | none | two plain-text lines: `gws <version>`, then the fixed disclaimer added in 0.3.5 |
...
| `gws alertcenter:v1beta1 alerts list --params <json>` | `<api>:<version>` form: `parse_service_and_version` splits off the version suffix, but `resolve_service` still rejects any API absent from the `SERVICES` table, so this command fails with exit 3 on a build without an `alertcenter` alias (see the Alert Center caveat above); parameters from [alerts.list](https://developers.google.com/workspace/admin/alertcenter/reference/rest/v1beta1/alerts/list) | `--params` with `pageSize`, optional `filter` | `alerts[].alertId`, `type`, `source`, `createTime`, `updateTime`, `metadata.status`, `metadata.severity`, `metadata.assignee`, `nextPageToken` |
