...
- A count judged over a list whose pagination stopped early renders `null` as well, never the count of the part read, with the seen-of-total beside it: the identity summary's `active_users_without_mfa`, `locked_users`, `dormant_active_users`, and `active_users_without_last_login` while `users_complete` is `false`; `active_audit_indexes` and SUMO-09's `active_audit_index_partitions` while `partitions_complete` is `false`; `external_email_recipients` while `monitors_complete` is `false`; and SUMO-12's `installed`, `hosted`, and `ephemeral` collector counts while the collector list is incomplete. An empty set that came from a partial read is unread rather than absent: when the audit policy is on but no active `AuditIndex` partition was among the partitions seen before pagination stopped, SUMO-09 renders `manual` with `no active AuditIndex partition was among the N partitions seen before pagination stopped, so the audit index is unread rather than absent` instead of the plan-availability note a complete list earns.
...
| 10 | Data Forwarding Destinations Reviewed | data governance | SUMO-10 | `manual` when the connection list is unreadable; `pass` on zero connections and zero forwarding destinations only when the partition and scheduled view lists were both readable and the partition list is non-empty, or when at least one connection exists and every connection host matches `approved_destination_domains`; `fail` on unapproved hosts; `manual` when an approved list was supplied but no connection exists to check (the summary counts the forwarding destinations on partitions and scheduled views that remain unchecked, or states that an empty partition inventory cannot confirm their absence); `manual` when no approved list was supplied; a page-capped connection, partition, or scheduled view list drops a pass to `warn` naming the list and its seen count (`evidence.incomplete_inventories`); an unreadable partition or scheduled view list drops a pass to `manual` naming that list under `Not checked:` |
