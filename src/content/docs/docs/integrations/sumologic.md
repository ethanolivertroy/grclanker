...
- `_errors.log` lists `<area>: <dataset>: <error>` with the access key removed from every message; `analysis/<area>.json` holds the assessment summary, findings, and errors; `QUICK_REFERENCE.md` and the `compliance/` reports hold finding text only.
...
- Counts and lists derived from an unreadable inventory render `null` in finding evidence and in the assessment `summary`, never `0` or `[]`.
