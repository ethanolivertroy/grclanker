...
Unreadable endpoints, empty inventories, capability-limited views, and unfollowed pagination never produce `pass`; a finding that reads several inventories never passes while one of them is unreadable (the summary names it under `Not checked:`), pagination that stops on the page cap, a repeated cursor, or an empty page with a next token is reported incomplete, and `core_data/` snapshots and assess payloads are allowlist projections with credential values replaced by `[REDACTED]`.
