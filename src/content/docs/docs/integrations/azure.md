...
| Tool | Findings |
|------|----------|
| `azure_check_access` | Probes eight Graph and ARM surfaces and reports which are readable; a probe that stopped at its page cap or at a refused next link carries `truncated: true` (with the refusal as `truncation`) and its count is a floor |
...
- Every pagination walk reports `truncated: true` on each early exit: the item cap, a next link that repeats the page just fetched, an empty page that still advertises a next link, and a next link that leaves the configured origin. Truncated pages cap `pass` at `warn` with seen and total counts (AZURE-MON-04 and AZURE-MON-05 included), and `azure_check_access` keeps the flag next to a capped probe count so `core_data/access.json` never presents a page cap as an inventory size.
- A server-supplied next link (`@odata.nextLink`, `nextLink`) is followed only when it resolves onto the configured Graph or ARM origin: same scheme, host, and port, and no userinfo. A link to another origin, a protocol-relative `//host/...` link, a userinfo-bearing link, or one that does not parse is refused before any request is made for it, so the bearer token never leaves for it; the page is `truncated: true` with a fixed-text `truncation` reason (`the API advertised a next page on another origin (scheme, host, or port), so the link was not followed and no request was made for it`, or the userinfo and unparseable variants) that reaches the finding summary, its evidence, the access surface, and the access note. No part of the link (host, path, query, userinfo) is recorded. The same rule sits in front of the transport, where a refused target renders as `Request refused: the target is not on the configured origin, so no request was made.` with `request_url` set to the configured base.
...
- Graph and ARM list walks stop at 5,000 items (500 for risk detections, 200 for alerts, audits, and sign-ins, 20 for secure scores, `max_assignments` for role assignments), on a repeated or empty-page next link, and on a next link off the configured origin (refused unrequested); every early exit is reported as truncated and demotes the dependent verdict.
