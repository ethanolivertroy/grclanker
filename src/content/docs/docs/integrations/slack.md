...
Token format evidence: `SLACK-APP-07` records `token_format` (`prefix` and `rotating_format`) from the configured token only when the token has the documented `xox?-` shape (`xoxb`, `xoxp`, `xoxe.xoxp`; https://docs.slack.dev/authentication/tokens); a value outside that shape (a misconfigured value, a proxy or gateway token, a pasted OAuth code, an `xapp-` or `xwfp-` token) renders the fixed prefix `unknown` with `rotating_format: false`, and no part of the value is echoed in evidence.
