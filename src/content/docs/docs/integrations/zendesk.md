...
That pass draws one boundary. A value inside a carrier is replaced with `[REDACTED]` whatever its shape: `Authorization`, `Proxy-Authorization`, `Cookie`, `Set-Cookie`, `X-Api-Key`, and similar header lines; cookie and session assignments (`_zendesk_session=...`); URL userinfo and `token=`, `api_key=`, `client_secret=`, `access_token=`, `signature=`, `pass=` query pairs; the values after the schemes `Bearer`, `Basic`, `Token`, and `ApiKey`; credential-named `key=value`, `key: value`, `"key": "value"` pairs and XML attributes. A configured secret (the API token, the OAuth token, and the base64 `{email}/token:{api_token}` value the Basic header carries, four characters or longer) is replaced wherever it appears, whatever its shape, in its base64, base64url, URL-encoded, and JSON-escaped forms too. Real token shapes go even when bare in prose: runs of sixteen or more characters with base64 symbols, scattered digits, or token casing, hex digests, JWTs, AWS access key ids, and PEM blocks. A bare value shaped like a name (words joined by hyphens or underscores with at most one numeric segment, such as `prod-us-east-2026` or `acme-support-01`) standing alone in prose is indistinguishable from a resource name and stays, as do titles such as `Basic Support Plan`, count fields such as `"oauth_tokens": 1`, and this integration's own vocabulary (`api_token_access`, `password_policy`, `status_code`). Every bundle file and every tool result passes through the configured-secret pass a final time on its way out, so a token echoed by an upstream body cannot survive even where the general scrub would have left evidence alone. The module's own fixed text is worded to cross that boundary intact: an `_errors.log` line reads `oauth_tokens dataset: <error>`, never `oauth_tokens: <error>`, which the pair rule would read as a credential and replace, and the test suite drives every fixed text the module emits on refused, unavailable, failed, and silent-success paths through the scrub and requires it back unchanged.

A snapshot that was refused, not on the plan, or failed is never written as an empty list or `null`. Its `core_data/` file holds a marker in its place, `{ "collected": false, "status": <observed HTTP status or null>, "dataset_status": "forbidden" | "not_found" | "error", "endpoint": "GET /api/v2/...", "error": "<scrubbed message>" }`, and every assessment summary carries a `collection` object with one entry per snapshot it read (`status`, `endpoint`, `http_status`, `seen`, `truncated`, `pages`, `error`). `truncated` is derived from the observed page walk (the reasons listed under the tools below), never from a default, and `seen`, `truncated`, and `pages` render `null` for a snapshot that was not read. Every endpoint or status code named anywhere in a finding, summary, tool result, or `_errors.log` line is the request that actually failed (its method and path are taken from the thrown error) and the status it actually returned.
...
| 15 | Marketplace apps reviewed for permissions | `zendesk_assess_integrations` | `ZD-15` | `pass` when the installation inventory is readable, complete, and empty and the owned-apps list (used to tell private apps from marketplace apps) was readable; `warn` when zero installations were seen but the list was truncated, or when the owned-apps list was unreadable (the pass is capped and the cause named); `manual` with the inventory when apps are installed. |
...
- A finding that reads several inventories never passes on a partial view: when a secondary inventory it needs (Account Settings for ZD-12 and ZD-21, custom roles for ZD-12, the token audit log for ZD-13, OAuth tokens for ZD-14, owned apps for ZD-15, targets or webhooks for ZD-24 and ZD-25) is unreadable or truncated, the verdict is capped at `warn` with the cause named and `verdict_capped_by_unreadable` listed in evidence. Counts and lists derived from an inventory that could not be read render as `null` in finding evidence and assessment summaries, never `0` or `[]`, and ZD-02's named-principal lists carry a `_partial` marker when the team list was truncated.
