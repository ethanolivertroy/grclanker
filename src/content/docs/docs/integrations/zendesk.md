...
Error strings follow the same discipline at the single place they are built and again at every sink. A non-2xx response with a non-JSON body (an HTML proxy page, an SSO interstitial, a rate-limit page) is described only as `{status} {statusText}; non-JSON {content-type} response body ({n} bytes, not echoed)`, whatever content type the server claimed; a JSON body contributes only Zendesk's documented error fields (`error`, `description`, `message`, `error.title`, `error.message`, `errors[].title` or `detail`), each scrubbed before it is shortened to 200 characters. The `ZendeskApiError` constructor, the transport-failure path, the snapshot layer that feeds every finding and `core_data/` file, and the tool catch blocks all apply an unanchored pass that replaces `Bearer` and `Basic` values, `Authorization`, `Cookie`, `Set-Cookie`, and `X-Api-Key` header lines, `key=value` pairs whose key contains `session`, `token`, `secret`, `password`, `api_key`, `signature`, or `authorization`, JWT-shaped strings, and URL userinfo, so a message assembled at a throw site cannot carry a credential into a finding, summary, `_errors.log`, or the bundle.
