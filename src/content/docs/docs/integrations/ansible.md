...
- A `next` link is followed only when it resolves onto the configured `AAP_URL` origin: AAP's root-relative `/api/v2/...?page=2` form and the same absolute origin pass; a link to another host, port, or scheme, a protocol-relative `//host/...` link, a userinfo-bearing link, or one that does not parse is refused before any request is made for it, so neither the token nor the session cookie leaves for it. The collection is `complete: false` with a fixed-text `truncation` reason (`the API advertised a next page on another origin (scheme, host, or port), so the walk was stopped; the link was not followed and no request was made for it`, or the userinfo and unparseable variants) and no part of the link is recorded. The same rule sits in front of the transport, ahead of the session login: a refused target renders as `AAP request refused: the target is not on the configured origin, so no request was made.` with `endpoint` set to `not requested`. The survey spec and error notification probes (50 templates each) report `<n> of <eligible> probed` the same way for controls 18 and 27.
