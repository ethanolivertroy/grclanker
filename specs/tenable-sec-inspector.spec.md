...
- Control 1 reads `GET /policies/{policy_id}` for every policy referenced by a scan (`policy_id` on the scan object) and judges the documented `settings.safe_checks`, `settings.portscan_range`, and `plugins` family map (`status` enabled or disabled). Performance settings (`max_hosts_per_scan`, `max_checks_per_host`, `thorough_tests`, `report_paranoia`) are recorded as evidence only. The details read requires the Standard [32] role plus Can View on each template; a refused read renders the control `manual`, and at most 100 referenced policies are read per run (more flags a partial view).
- Control 11 treats subject type `AllUsers` and the tenant-wide `UserGroup` `00000000-0000-0000-0000-000000000000` (All Users) as broad subjects, and `AllAssets`, `AllObjects`, and `AllTags` as broad objects, including Tenable's default system-created All Assets permission when it grants CanScan.
- Control 19 report schedules have no documented read endpoint; the control relies on export job history from `GET /vulns/export/status` and `GET /assets/export/status`, which list completed jobs only from the previous three days, so the window is three days and the finding states that it is a point-in-time signal. Jobs created by this tool are excluded by UUID and by export shape.
- Control 8 evaluates `loaded_plugin_set` on every scanner entry that exposes one (cloud scanners included); when no entry exposes a plugin set the control is `manual`, never `pass`.
- Control 10 requires at least one user with `enabled=true` before it can pass; users without an `enabled` flag are not counted as enabled.
