...
- Bundle layout: `core_data/` (snapshots projected to the documented fields the verdicts read, then redacted, each with error, truncation, and page metadata), `analysis/` (`findings.json` plus per-category JSON and Markdown), `compliance/` (`executive_summary.md`, `unified_compliance_matrix.md`, one report per framework), `QUICK_REFERENCE.md`, `_errors.log` on partial collection, and a zip named after the allocated directory; reruns allocate `-2`, `-3`.
