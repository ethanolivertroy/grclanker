...
- `/api/v2/tokens` returns only the caller's tokens unless `showAll=true` is passed by an Admin or Owner token (controls 8 through 11). `/api/v2/caller-identity` exposes no role, so the inspector derives the caller's base role from its own token and member records in the listing, treats other members' personal tokens as proof of a complete inventory, resolves an Admin personal caller whose member record cannot be found (members unreadable or truncated) or an empty listing to unknown, and never passes controls 8 through 11 on a partial or unknown inventory, including one that returned zero tokens (clean results degrade to `warn` with the caveat in the summary).
- Listing limits are verdict boundaries. Every paginated collection (members, teams, team roles, custom roles, tokens, projects, environments, SDK keys, flags) reports whether items remained beyond the configured limit (`_links.next` still present or `totalCount` above the collected count). Findings that depend on a truncated collection never `pass`: they degrade to `warn`, state the seen versus total counts and the argument to raise, and carry `truncated_collections` evidence; the `core_data/` snapshots record `truncated`, `seen`, and `total` per collection. Audit log queries are presence probes and are not treated as truncation.
- Control 17 evaluates the full documented `approvalSettings` object, not just `required`. `bypassApprovalsForPendingChanges`, `canReviewOwnRequest`, `canApplyDeclinedChanges` (apply after one approval even when others declined), and a non-empty `requiredApprovalTags` list (untagged flags skip approval) each weaken the gate to `warn`; `minNumApprovals` is recorded as evidence because its documented minimum is 1.
