...
- 61 mocked tests in `cli/tests/newrelic.test.mjs`, including one regression test per verdict-safety rule, four false-pass self-checks (all surfaces forbidden, all inventories empty, partial inventory, flag-stripped fixture) that assert no finding passes, one regression test per compliance-review fix (documented role catalog, layered `keySearch`, widened schema-mismatch matcher, live URL payload checks, trimmed notification fields with control 10 on `workflowEnabled`, manual control 6 on zero keys, the selection allowlist, and the group-grant fallback), plus a live smoke script (`npm --prefix cli run test:newrelic:live`) that skips without credentials.
