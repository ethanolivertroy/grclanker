...
- Six read-only tools (`newrelic_check_access`, four `newrelic_assess_*` tools, `newrelic_export_audit_bundle`) covering all 20 controls in section 4. Controls that the API cannot establish produce `manual` findings that name the exact UI evidence to collect: 16 (agent transport configuration) and 18 (session and user upgrade settings) always, 17 unless an enrichment routes to an external destination, 20 whenever custom roles exist, 6 whenever keys exist, and 1 when `authenticationType` is not exposed.
- Verdict safety: no finding passes on missing or partial evidence. Unreadable or errored surfaces (including GraphQL `errors` with partial `data`) yield `manual` with the cause and the evidence to collect; empty inventories yield `manual` or `fail` per the control's intent (the only accepted empty passes are control 6 with complete `keySearch` coverage of every visible in-scope account and control 20 with a complete role listing that includes standard roles, both stated in the summary); scoped-out or not-applicable controls yield `manual`; items without `lastActive` or `createdAt` are bucketed separately and cap the verdict at `warn`; truncated pagination, unreadable scopes, the `keySearch` single-page fallback, and script sampling at the limit cap an otherwise passing finding at `warn` with a `Partial view:` clause; every flag a verdict depends on (`authenticationType`, user `type`, role grants, condition `enabled`, `workflowEnabled`, destination `type` and `active`, obfuscation `enabled`, `retentionInDays` per namespace) is read explicitly. Each assessment result lists its `coverage` limitations, which the bundle repeats in the analysis JSON and executive summary.
...
- NerdGraph cursor pagination that runs to completion or records truncation, REST v2 `Link` header and `page=` pagination, retry with backoff on 429 and 5xx honoring `Retry-After` and `X-RateLimit-Reset`, request timeouts, and API key redaction in errors.
...
- 48 mocked tests in `cli/tests/newrelic.test.mjs`, including one regression test per verdict-safety rule and three false-pass self-checks (all surfaces forbidden, all inventories empty, partial inventory) that assert no finding passes, plus a live smoke script (`npm --prefix cli run test:newrelic:live`) that skips without credentials.
...
- `apiAccess.keySearch` is paginated with `cursor` when the schema accepts it and falls back to a single page otherwise; the fallback is recorded as an incomplete listing and caps key-hygiene findings at `warn`. The documentation notes that user keys belonging to other users are returned with obfuscated values, so the inventory covers every visible key while key values are never requested.
