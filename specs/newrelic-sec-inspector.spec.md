...
- 70 mocked tests in `cli/tests/newrelic.test.mjs`, including one regression test per verdict-safety rule, four false-pass self-checks (all surfaces forbidden, all inventories empty, partial inventory, flag-stripped fixture) that assert no finding passes, one regression test per compliance-review fix (documented role catalog, layered `keySearch`, widened schema-mismatch matcher, live URL payload checks, trimmed notification fields with control 10 on `workflowEnabled`, manual control 6 on zero keys, the selection allowlist, and the group-grant fallback), a planted-credential bundle test that reads back every bundle file and every zip entry, per-surface truncation tests for a stalled cursor, an empty page beside a cursor, and a listing short of its total, plus a live smoke script (`npm --prefix cli run test:newrelic:live`) that skips without credentials.
