...
- Verdict safety: an unreadable or errored endpoint never yields `pass` (the control becomes `manual` naming the failed read); empty inventories `fail` or become `manual` unless emptiness is compliant (exclusions, alerts in the stated window, contained hosts); undated records are bucketed separately and cap the verdict at `warn`; every sampled or paginated read compares the returned count against `meta.pagination.total` and a truncated inventory caps the verdict at `warn` with seen and total counts (single-page id queries such as the role catalog and Identity Protection rules included), while count-only reads that omit the total (Discover, ZTA) become `manual` or `warn` instead of counting as zero; only enabled policies assigned to host groups (or `platform_default`) count as enforcement; and re-running the exporter allocates a new bundle directory with a matching zip name instead of overwriting.
...
- CS-01: the API has no `MLSliderDetectionLevels` or `MLSliderPreventionLevels` settings. ML levels are per-slider `mlslider` settings with `detection` and `prevention` values; the implementation evaluates `CloudAntiMalware` and `OnSensorMLSlider` and reports the other paired sliders present in the Terraform provider schema (`AdwarePUP`, `CloudAntiMalwareForMicrosoftOfficeFiles`, `CloudMLSliderForPupAdwareCloudEndUserScans`, `OnSensorMLAdwarePUPSlider`, `OnSensorMLSliderForSensorEndUserScans`, `OnSensorMLSliderForCloudEndUserScans`) as evidence.
...
- CS-18: API clients live in the `api-clients` collection (`/api-clients/queries/api-clients/v1`, `/api-clients/entities/api-clients/v1`), not User Management. Scope objects carry the read or write verb in `action` alongside `group` and `id` (the `msaspec.Scope` shape documented for `GetAccessibleScopes`), so write detection reads `action` and `group`; a scope without an `action` is reported as unknown and caps the verdict at `warn`. The public reference documents no last-used field for API clients, so the unused-client half of this control is not evaluated from API data and the finding says so.
