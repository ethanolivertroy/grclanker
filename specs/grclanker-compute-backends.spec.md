...
| `runpod-pod` | yes (HTTP `GET /pods/{id}` plus SSH/scp) | contract adapter | yes (mocked fetch, injected runner, scp-failure cleanup, session-id validation, awaited teardown, redaction, provider error-body table over the pod lookup and ssh/scp stderr) | yes | REST v1 is deprecated by RunPod; base URL isolated for the v2 move |
