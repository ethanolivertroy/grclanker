...
| `runpod-pod` | yes (HTTP `GET /pods/{id}` plus SSH/scp) | contract adapter | yes (mocked fetch, injected runner, tracked-files staging plan against a real temp repo with planted secrets, scp-failure cleanup, cleanup exit-code tracking at adapter, runtime, sweep, and exit-hook level, session-id validation, awaited teardown, redaction, provider error-body table over the pod lookup and ssh/scp stderr) | yes | stages the git index only (tracked files minus the `RUNPOD_STAGING_DENYLIST` paths; ignored, untracked, `.git`, symlinks, and submodule contents never leave the machine; a non-git workspace is refused); REST v1 is deprecated by RunPod; base URL isolated for the v2 move |
