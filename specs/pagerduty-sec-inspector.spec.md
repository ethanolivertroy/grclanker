...
- Enabling flags (`is_enabled` on workflows, `is_disabled: false` on workflow triggers, `active`, `enabled`, `blacklisted`, `num_loops`, rule `targets`, user `role`, `extension_schema`, ability names) must be present and true; an absent or false flag never supports `pass`. Control 10 counts only triggers with `is_disabled: false`, reports disabled and unflagged trigger counts in the summary, and caps at `warn` when any trigger lacks the flag.
...
- Control 10: `GET /response_plays` is no longer in the published OpenAPI reference, so the implementation reads `GET /incident_workflows` and `GET /incident_workflows/triggers` and treats any `response_play` reference still present on a service as legacy automation. The trigger `is_disabled` boolean is marked deprecated in the OpenAPI reference (it mirrors the owning workflow's `is_enabled`); while it is served, a trigger passes only with `is_disabled: false`, and a trigger without the flag is unverifiable and caps the finding at `warn`.
