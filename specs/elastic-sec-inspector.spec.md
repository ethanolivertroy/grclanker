...
- `elastic_check_access`, five `elastic_assess_<area>` tools, and `elastic_export_audit_bundle`, together covering all 23 Section 4 controls with the Section 5 mappings on every finding. Controls whose endpoints are unreadable degrade to `manual` findings that name the evidence a human must collect.
- 34 mocked Node tests plus a live smoke script that skips cleanly without credentials.
...
- Added surfaces not in Section 2 that the controls require: `GET /_security/_authenticate`, `POST /_security/user/_has_privileges`, `GET /_ilm/policy`, `GET /_slm/policy`, `GET /_snapshot/_all`, `POST /_watcher/_query/watches`, `GET /_ingest/pipeline`, `GET /api/fleet/fleet_server_hosts`, `GET /api/detection_engine/rules/_find`, `GET /api/alerting/rules/_find`, and `GET /api/actions/connectors`.
