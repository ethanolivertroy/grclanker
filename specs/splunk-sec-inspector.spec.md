...
- Verdict safety: forbidden or errored endpoints, empty inventories, partial inventories (`paging.total` above the entries returned), out-of-scope controls, and items without dates never yield `pass`. A documented explicit value always beats an inference: names, leftover certificate settings, and passwords can at most produce `warn` and never upgrade an explicit negative; roles that do not expose `srchIndexesAllowed` are reported as unknown rather than counted as restricted. Rerunning the export allocates a new directory and a zip named after it, so prior bundles are never overwritten.
