...
- Export bundle: `core_data/` (projected snapshots: identifiers plus the documented fields each control reads, never whole resources, metadata values, labels, key material, or other free-form values), `analysis/` (`findings.json`, `category_summaries.json`, per-category JSON and markdown, with evidence limited to derived values), `compliance/` (`executive_summary.md`, `unified_compliance_matrix.md`, `frameworks/<framework>.md` for the eight frameworks in the mapping table), `QUICK_REFERENCE.md`, `_errors.log` only when collection partially failed, and a zip named after the allocated directory (`-2`, `-3` on rerun).
