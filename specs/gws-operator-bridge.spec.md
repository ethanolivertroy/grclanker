...
- 5 of 5 workflows: `gws_ops_check_cli`, `gws_ops_investigate_alerts`, `gws_ops_trace_admin_activity`, `gws_ops_review_tokens`, `gws_ops_collect_evidence_bundle`, each with `dry_run` previews and the exact executed command in the result. A dry run renders null for `count`, `complete`, `nextPageToken`, `records`, and `raw` beside a `not collected` status saying the command was previewed and not executed (rule 11); an executed call carries a `complete` or `partial: at least N` status derived from the trailing `nextPageToken`.
...
- Bundle secret hygiene (rule 9): `raw/<category>.json` is never the verbatim CLI stdout. Each page is projected to the documented Alert Center and Reports fields above (dropping `data`, `events[].parameters[]`, `actor.key`, `securityInvestigationToolLink`, and undocumented keys), then `raw/` and the records pass through the inspector's `redactSecrets` and a scrub of every `GOOGLE_WORKSPACE_CLI_*` token or secret value present in the environment. An end-to-end test drives a scripted `gws` that echoes its own `GOOGLE_WORKSPACE_CLI_TOKEN` and secret-bearing records, then greps every bundle file and zip entry. Error text is covered by the same rule: CLI stderr and every error string pass through `scrubCliText` (`gws-ops.ts`), which runs the inspector's `scrubText` and then replaces the environment's `GOOGLE_WORKSPACE_CLI_*` values; `GwsCliCommandError` scrubs its message in its constructor, the access probe scrubs the stderr it reports, a JSON parse failure is described without repeating stdout, and each tool's error result is scrubbed once more where it is built. A test plants an env token, a `Bearer ya29.` value, and an `access_token=` pair in stderr and asserts none reaches the thrown error, the tool result, or the probe. The `--version` stdout passes through `scrubCliText`, its first line is matched against the documented `gws <version>` line (the binary prints that line and then a fixed disclaimer, `crates/google-workspace-cli/src/main.rs`), and only the numeric core `major.minor.patch` is rendered, a pre-release or build suffix being accepted but dropped; output whose first line is not the documented one renders a bare version core plus a note or a descriptor that gives only the output length. Tests pin the real two-line output byte for byte, run a scripted `gws` whose version string echoes `$GOOGLE_WORKSPACE_CLI_TOKEN`, and put a 20-character fragment of that token in the pre-release position, asserting neither reaches the tool text or its JSON.
