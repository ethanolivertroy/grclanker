...
- SQL REST API client with async submit, `202` polling honoring `Retry-After`, full partition fetching, `429`/`5xx` retry with backoff, HTTP timeouts, a read-only statement guard, and redaction of tokens, JWTs, private keys, and passphrases from errors.
- Verdict safety: denied, failed, or timed-out statements yield `manual`; empty inventories yield `fail` or `manual` per control intent (only controls 10, 16, 22, and 23 can pass on a readable empty result, the last two only under `ACCOUNTADMIN`/`SECURITYADMIN`); truncated partitions, row limits, and custom roles downgrade `pass` to `warn`; NULL `LAST_SUCCESS_LOGIN` is bucketed separately and never counted as active; exports never overwrite a prior bundle.
- 35 regression tests including the three false-pass self-check fixtures (all statements denied, all result sets empty, partial results), plus a live smoke script.
...
- `NETWORK_POLICY_REFERENCES`, `MASKING_POLICY_REFERENCES`, and `ROW_ACCESS_POLICY_REFERENCES` (section 2) are not `ACCOUNT_USAGE` views; assignments are read from `ACCOUNT_USAGE.POLICY_REFERENCES` filtered by `POLICY_KIND`, and account-level network policy activation is read from `SHOW PARAMETERS LIKE 'NETWORK_POLICY' IN ACCOUNT`.
- Control 4 reads `ACCOUNT_USAGE.PASSWORD_POLICIES` plus `POLICY_REFERENCES` (`REF_ENTITY_DOMAIN = 'ACCOUNT'`) rather than `SHOW PARAMETERS`; control 25 reads `ACCOUNT_USAGE.SESSION_POLICIES` the same way.
...
- `SHOW SHARES` returns an empty result (not an error) without the `IMPORT SHARE` privilege, so control 22 only passes on zero outbound shares under `ACCOUNTADMIN` or `SECURITYADMIN`.
